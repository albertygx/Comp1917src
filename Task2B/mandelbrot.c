...
static char* locateChar(char *str, char needle);
...
int main(int argc, char* argv[]) {
    ...
    while (numberServed < NUMBER_OF_PAGES_TO_SERVE) {
        ...
        // only when requesting a bmp file
        if (strstr(request, ".bmp") != 0) {
            char *c = 0;
            x = strtod(locateChar(request, 'x') + 1, &c);
            y = strtod(locateChar(request, 'y') + 1, &c);
            z = strtod(locateChar(request, 'z') + 1, &c);
            int d = pow( 2, -z);
            double px = x - (IMAGE_SIZE/2 - 0.5)*d;
            double py = y - (IMAGE_SIZE/2 - 0.5)*d;
            // STEP 3. send the browser a simple html page using http
            printf(" *** Sending http response ***\n");
            serveHTML(connectionSocket);
            send(connectionSocket, header, HEADER_SIZE, 0);
            int i = 0;
            while (i < IMAGE_SIZE){
                int j = 0;
                while(j < IMAGE_SIZE){
                    char *buffer = {escapeSteps(px + j*d, py + i*d),
                            escapeSteps(px + j*d, py + i*d), escapeSteps(px + j*d, py + i*d)};
                    j++;
                    }
                i++;
                }
...
int makeServerSocket(int portNumber) {
    ...
    int bindSuccess = bind(serverSocket, (struct sockaddr*) &serverAddress,
...
//Find first occurrence of char [c] in string str,
//and return the index of it.
//return    pointer to the char if c is found.
//          0(null pointer) if not found.
static char* locateChar(char *str, char c) {
    char *pos = 0;
    while (!*str == 0) {
        if (*str == c) {
            pos = str;
