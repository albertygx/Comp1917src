...
int main(int argc, char *argv[]) {
    ...
    bits8 white = WHITE;
    while (pos < numBytes) {
        //If the pixel is at even row of squares
        if (pos % (2 * BYTES_PER_PIXEL * squareSize * SIZE) >=
        BYTES_PER_PIXEL * squareSize * SIZE ) {
            //If the pixel is in even column of squares
            if (pos % (2 * BYTES_PER_PIXEL * squareSize)
                    >= BYTES_PER_PIXEL * squareSize) {
                fwrite(&black, sizeof black, 1, outputFile);
                //If the pixel is in odd column of squares
            } else {
                fwrite(&white, sizeof white, 1, outputFile);
            }
            //If the pixel is at odd row of squares
        } else {
            //If the pixel is in even column of squares
            if (pos % (2 * BYTES_PER_PIXEL * squareSize)
                    >= BYTES_PER_PIXEL * squareSize) {
